...
export const AdminEventsPage = () => {
  ...
  const loadCategories = async () => {
    try {
      const response = await getCategories({ includeInactive: true });
  ...
  const handlePublish = async (eventId: number) => {
    try {
      ...
      setSuccessMessage(response.message || "เผยแพร่กิจกรรมสำเร็จ");
      await loadEvents();
    } catch {
      setErrorMessage("เผยแพร่กิจกรรมไม่สำเร็จ");
    }
  };
    ...
    <section className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-5 md:py-8 space-y-6">
      <div className="rounded-2xl bg-surface border border-border p-4 md:p-6 shadow-sm">
        ...
        <p className="text-sm md:text-base text-muted mt-2">
          ฝั่งผู้ดูแลระบบสามารถสร้าง แก้ไข เผยแพร่ และยกเลิกกิจกรรมได้
        </p>
      ...
      <div className="rounded-2xl bg-surface border border-border p-4 md:p-6 shadow-sm">
        <div className="mb-4 flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
          <h2 className="text-2xl font-semibold text-foreground">รายการอีเวนต์ทั้งหมด</h2>
                  ...
                  <tr key={event.eventId} className="border-t border-border text-sm">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex flex-wrap items-center gap-2">
                        <button type="button" onClick={() => handleEdit(event.eventId)} className="inline-flex items-center gap-1.5 h-8 px-2.5 rounded-lg border border-indigo-200 bg-indigo-50 text-indigo-700 text-xs font-semibold hover:bg-indigo-100">
                          <FiEye className="text-sm" />
                          ดู/แก้ไข
                        </button>
                        <button type="button" onClick={() => handlePublish(event.eventId)} className="inline-flex items-center gap-1.5 h-8 px-2.5 rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-700 text-xs font-semibold hover:bg-emerald-100">
                          <FiCheckCircle className="text-sm" />
                          Publish
                        </button>
                        <button type="button" onClick={() => handleCancel(event.eventId)} className="inline-flex items-center gap-1.5 h-8 px-2.5 rounded-lg border border-rose-200 bg-rose-50 text-rose-700 text-xs font-semibold hover:bg-rose-100">
